...
impl Ptmalloc {
    ...
    pub fn inuse(&self, p: &MallocChunk) -> bool {
        // Get next chunk and check its PREV_INUSE bit
        let _next_chunk_addr = p.address + (p.size & !self.size_bits);
        // For now, we'll handle this in the walk_heap method directly
        true // Placeholder - actual logic is in walk_heap
    }
...
impl Ptmalloc {
    pub fn print_heap(&self, heap_start_addr: u64) {
        let chunks = self.walk_heap(heap_start_addr);

        for (i, chunk) in chunks.iter().enumerate() {
            let is_last = i == chunks.len() - 1;

            // Check if this chunk is free by looking at next chunk's PREV_INUSE
            let is_free = if is_last {
                false
            } else if let Some(next_chunk) = chunks.get(i + 1) {
                !self.prev_inuse(next_chunk)
            } else {
                // For standalone check, look at chunk flags
            ...
            let flags = if self.prev_inuse(chunk) { " | PREV_INUSE" } else { "" };

            println!("{chunk_type}{flags}");
            println!("Addr: 0x{:x}", chunk.address);
            println!("Size: 0x{:x} (with flag bits: 0x{:x})", self.chunksize(chunk), chunk.size);

            if let (Some(fd), Some(bk)) = (chunk.fd, chunk.bk) {
                println!("fd: 0x{fd:x}");
                println!("bk: 0x{bk:x}");
            }
